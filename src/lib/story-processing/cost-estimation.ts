// src/lib/story-processing/cost-estimation.ts
// Cost estimation for story processing pipelines
 ...
 * Used for: Level detection, language detection
 */
...
export const DALLE3_PRICING = {
  ...
  standard_1024x1792: 0.08, // Portrait - thumbnails
  standard_1792x1024: 0.08, // Landscape - backgrounds
  hd_1024x1024: 0.08,
  hd_1024x1792: 0.12,
  hd_1792x1024: 0.12,
};
...
/**
 * All pricing in one object for convenience
 */
...
export interface CostEstimateOptions {
  ...
  /** Number of chapters (affects chunking overhead) */
  chapterCount?: number;
}

/**
 * Simplified options for user pipeline (auto-generates all levels)
 */
export interface SimpleCostEstimateOptions {
  /** Raw text content */
  rawText: string;
  /** Source language */
  sourceLanguage: "en" | "es";
  /** Detected source level (1-6) */
  sourceLevel: number;
  /** Number of levels to generate (default: 5 for user, A1-C1) */
  levelCount?: number;
  /** Number of chapters */
  chapterCount?: number;
}
...
function estimateRewriteCost(
  ...
  // Output is typically similar length, maybe shorter for lower levels
  const outputTokens = Math.ceil(
    textTokens * (targetLevel <= 2 ? 0.85 : 0.95)
  );
...
function estimateTranslateCost(
  ...
  const outputMultiplier = sourceLanguage === "en" ? 1.12 : 0.9;
...
/**
 * Generate a complete cost estimate for the pipeline (full options)
 */
export function estimateCosts(options: CostEstimateOptions): CostEstimate {
  ...
  // Calculate per-level costs (levels 1-6)
  ...
  const detectionTokens = {
    input: estimateTokensWithOverhead(
  ...
  if (generateMetadata) {
    const titleInput = estimateTokensWithOverhead(
      rawText.slice(0, 2000),
      sourceLanguage,
      400
    );
    const titleOutput = 300;
    const descInput = estimateTokensWithOverhead(
      rawText.slice(0, 2000),
      sourceLanguage,
      400
    );
    const descOutput = 600;

    metadataTokens = {
      input: titleInput + descInput,
      output: titleOutput + descOutput,
    };
    metadataCost = calculateCost(
  ...
  const totalInputTokens =
    levels.reduce(
      (sum, l) => sum + l.rewriteTokens.input + l.translateTokens.input,
      0
    ) +
    detectionTokens.input +
    metadataTokens.input;
  ...
  const totalOutputTokens =
    levels.reduce(
      (sum, l) => sum + l.rewriteTokens.output + l.translateTokens.output,
      0
    ) +
    detectionTokens.output +
    metadataTokens.output;
  ...
  if (totalCost > 5) {
    warnings.push(
      `Estimated cost exceeds $5.00. Consider reducing levels or using "use-original" for some.`
    );
  }
  ...
  return {
    levels,
    images,
    detection: { tokens: detectionTokens, cost: detectionCost },
    metadata: { tokens: metadataTokens, cost: metadataCost },
    totals: {
      rewriteCost,
      translateCost,
      imageCost,
...
/**
 * Simplified cost estimation for user pipeline.
 * Automatically generates all levels from A1 to C1 (5 levels).
 */
export function estimateUserStoryCost(
  options: SimpleCostEstimateOptions
): CostEstimate {
  const { rawText, sourceLanguage, sourceLevel, levelCount = 5, chapterCount = 1 } = options;

  // Build level modes: generate all levels except source level
  const levelModes: Record<number, "generate" | "use-original" | "omit"> = {};
  for (let level = 1; level <= levelCount; level++) {
    levelModes[level] = level === sourceLevel ? "use-original" : "generate";
  }
  // Omit C2 for user stories
  levelModes[6] = "omit";
  ...
  return estimateCosts({
    ...
    sourceLevel,
    levelModes,
    generateThumbnail: true, // User stories always get thumbnails
    generateBackground: false,
    generateMetadata: true, // User stories always get metadata
    chapterCount,
  });
}
