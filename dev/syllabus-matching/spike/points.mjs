// Shared syllabus grammar points + prompt text, extracted from match-spike-v2.mjs
// so the A/B harness feeds BOTH models byte-identical instructions.
// Any prompt change must happen here, never in a model-specific runner.
...
export const POINT_IDS = new Set(POINTS.map((p) => p.id));
...
export const SYSTEM = `You mark instances of syllabus grammar points in an English story for Spanish-speaking students (B2 course, B1-level text). Your output is reviewed by the teacher, then shown to students as highlights in a reading app.
...
/** Builds the user message for one page. Identical for every model. */
export function buildUserMessage(page, pageLines) {
